...
def load_train_data():
    df = pd.read_csv("../data/zhengqi_train.txt", header=0, sep="\s+")
    # print(df.describe())
...
def load_test_data():
    df = pd.read_csv("../data/zhengqi_test.txt", header=0, sep="\s+")
    # print(df.describe())
    X_test = df
    return X_test
...
def main():
    ...
    print("单变量选择后的shape")
    # pca_ = PCA(n_components=0.99) #0.95
    # pca_.fit(X)
    ...
    result.to_csv("../tmp/result(averaged_test_).txt", index=False, header=False)
    print("Predict Done.")
    print(datetime.now())
